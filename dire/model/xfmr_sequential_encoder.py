...
class TransformerModel(nn.Module):
    ...
    def __init__(self, ninp, nhead, nhid, nlayers, dropout=0.5):
        ...
        encoder_layers = TransformerEncoderLayer(nhid, nhead, nhid, dropout, activation="gelu")
    ...
    def forward(self, src, src_padding):
        ...
        masked_sum = (output.transpose(0, 1) * ((~src_padding).unsqueeze(-1).expand(-1, -1, self.nhid))).sum(dim=1)
        lengths = (~src_padding).sum(dim=1, keepdim=True)
        avg = masked_sum / lengths
        state = torch.stack([avg[:, :self.nhid // 2], avg[:, self.nhid // 2:]]).unsqueeze(0)
...
class XfmrSequentialEncoder(Encoder):
    ...
    def encode_sequence(self, code_sequence):
        ...
        sorted_encodings, (last_states, last_cells) = self.encoder(code_token_embedding.transpose(0, 1), (1 - code_token_mask).bool())
        ...
        # last_states = last_states.view(1, 2, -1, self.encoder.nhid // 2)
        # last_states = last_states.index_select(dim=2, index=restoration_indices)
        # last_cells = last_cells.view(1, 2, -1, self.encoder.hidden_size)
