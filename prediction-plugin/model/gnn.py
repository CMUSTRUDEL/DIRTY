...
class GatedGraphNeuralNetwork(nn.Module):
    def __init__(self, hidden_size, num_edge_types, layer_timesteps,
        ...
        self.residual_connections = {
            int(k): v for k, v in residual_connections.items()
        }
        ...
        for layer_idx in range(len(self.layer_timesteps)):
            ...
            for edge_type_j in range(self.num_edge_types):
                ...
                layer_name = \
                    ...
                    f'_type{edge_type_j}'
                self.state_to_message_linears[layer_name] = \
                    state_to_msg_linear_layer_i_type_j

            layer_residual_connections = \
                self.residual_connections.get(layer_idx, [])
            rnn_cell_layer_i = \
                nn.GRUCell(self.hidden_size
                           * (1 + len(layer_residual_connections)),
                           self.hidden_size)
            self.rnn_cells.append(rnn_cell_layer_i)

        self.state_to_message_dropout_layer = \
            nn.Dropout(self.state_to_message_dropout)
    ...
    def compute_node_representations(self,
        ...
        if init_node_repr_size < self.hidden_size:
            ...
            zero_pads = torch.zeros(node_num,
                                    pad_size,
                                    dtype=torch.float,
                                    device=self.device)
            initial_node_representation = \
                torch.cat([initial_node_representation, zero_pads], dim=-1)
            ...
            for t in range(num_timesteps):
                ...
                for edge_type_idx, adjacency_list_for_edge_type in \
                    ...
                    if adjacency_list_for_edge_type.edge_num > 0:
                        ...
                        # shape [E, D]
                        edge_source_states = \
                            node_states_for_this_layer[edge_sources]
                ...
                # shape [V, D]
                incoming_messages = torch.zeros(node_num,
                                                messages.size(1),
                                                device=self.device)
                incoming_messages = \
                    incoming_messages.scatter_add_(0,
                                                   message_targets
                                                   .unsqueeze(-1)
                                                   .expand_as(messages),
                                                   messages)
                node_target_num = \
                    torch.zeros(node_num, device=self.device).scatter_add(
                        0,
                        message_targets,
                        torch.ones(messages.size(0), device=self.device)
                    )
                incoming_messages = \
                    incoming_messages / (node_target_num + 1e-7).unsqueeze(-1)
                # shape [V, D * (1 + num of residual connections)]
                incoming_information = \
                    torch.cat(layer_residual_states + [incoming_messages],
                              dim=-1)
                ...
                updated_node_states = \
                    self.rnn_dropout_layer(updated_node_states)
...
def main():
    gnn = GatedGraphNeuralNetwork(hidden_size=64,
                                  num_edge_types=2,
                                  layer_timesteps=[3, 5, 7, 2],
                                  residual_connections={2: [0], 3: [0, 1]})

    adj_list_type1 = AdjacencyList(node_num=4,
                                   adj_list=[(0, 2), (2, 1), (1, 3)],
                                   device=gnn.device)
    adj_list_type2 = AdjacencyList(node_num=4,
                                   adj_list=[(0, 0), (0, 1)],
                                   device=gnn.device)
