...
def evaluate(config, most_common_for_src: Dict[int, int]):
    ...
    for example in tqdm(dataloader):
        for src_name, src_type in zip(example.src_var_names, example.src_var_types_str):
            results.setdefault(example.binary, {}).setdefault(example.name, {})[src_name[2:-2]] = most_common_for_src.get(src_type, src_type), ""
...
def compute(config) -> Dict[int, int]:
    ...
    for example in tqdm(dataloader):
        for src_type, tgt_type in zip(example.src_var_types_str, example.tgt_var_types_str):
